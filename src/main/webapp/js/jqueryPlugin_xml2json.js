...
alert( json1.test1[0].Text );	// 1
alert( json1.test1[0].name );	// asd
*/
...
if($) {
	...
	$.extend({
		textToXML: function(strXML) {
			try {
				// -----------------------------------------------------------
				// 비 IE 브라우저 일경우 xml 선언문이 오게되면 정상적인 XML이 만들어지지 않아서 모두 빼버렸다. by ddakker
				var firstIndex = strXML.indexOf("<?");								
				if( firstIndex > -1 ){
					var firstIndex = strXML.indexOf("?>");
					strXML = strXML.substring(firstIndex+2, strXML.length);
				}
				// -----------------------------------------------------------
